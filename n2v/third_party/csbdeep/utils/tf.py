...
class CARETensorBoard(Callback):
    ...
    def set_model(self, model):
        ...
        if self.compute_histograms and self.freq and self.merged is None:
            for layer in self.__wrapped_model.layers:
        ...
        n_inputs, n_outputs = len(self.__wrapped_model.inputs), len(self.__wrapped_model.outputs)
        ...
        self.gt_outputs = [K.placeholder(shape=_gt_shape(K.int_shape(x),sh)) for x,sh in zip(self.__wrapped_model.outputs,output_target_shapes)]
        ...
        for i,sl in zip(image_for_inputs,input_slices):
            # print('input', self.__wrapped_model.inputs[i], tuple(sl))
            layer_name = _name('net_input', self.__wrapped_model.inputs[i], i, n_inputs)
            input_layer = tf_normalize_layer(self.__wrapped_model.inputs[i][tuple(sl)])
        ...
        for i,sl in zip(image_for_outputs,output_slices):
            # print('output', self.__wrapped_model.outputs[i], tuple(sl))
            output_shape = self.__wrapped_model.output_shape if n_outputs==1 else self.__wrapped_model.output_shape[i]
            # target
            output_layer = tf_normalize_layer(self.gt_outputs[i][tuple(sl)])
            layer_name = _name('net_target', self.__wrapped_model.outputs[i], i, n_outputs)
            ...
            if self.prob_out: # first half of output channels is mean, second half scale
                ...
                sep = sep // 2
            output_layer = tf_normalize_layer(self.__wrapped_model.outputs[i][...,:sep][tuple(sl)])
            if self.prob_out:
                scale_layer = tf_normalize_layer(self.__wrapped_model.outputs[i][...,sep:][tuple(sl)], pmin=0, pmax=100)
                mean_name  = _name('net_output_mean',  self.__wrapped_model.outputs[i], i, n_outputs)
                scale_name = _name('net_output_scale', self.__wrapped_model.outputs[i], i, n_outputs)
                ...
                layer_name = _name('net_output', self.__wrapped_model.outputs[i], i, n_outputs)
                tf_sums.append(tf.summary.image(layer_name, output_layer, max_outputs=self.n_images))
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.validation_data and self.freq:
            if epoch % self.freq == 0:
                ...
                tensors = self.__wrapped_model.inputs + self.gt_outputs + self.__wrapped_model.sample_weights

                if self.__wrapped_model.uses_learning_phase:
...
class CARETensorBoardImage(Callback):
    ...
    def __init__(self, model, data, log_dir,
        ...
        self.__wrapped_model = model
        self.n_inputs  = len(self.__wrapped_model.inputs)
        self.n_outputs = len(self.__wrapped_model.outputs)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch == 0:
            with self.file_writer.as_default():
                for i,sl in zip(self.image_for_inputs,self.input_slices):
                    # print('input', self.__wrapped_model.inputs[i], tuple(sl))
                    input_name = self._name('net_input', self.__wrapped_model.inputs[i], i, self.n_inputs)
                    input_image = self._normalize_image(self.X[i][tuple(sl)])
                    tf.summary.image(input_name, input_image, step=epoch)

        # outputs
        Yhat = self.__wrapped_model.predict(self.X, batch_size=self.batch_size)
        if self.n_outputs == 1 and isinstance(Yhat,np.ndarray): Yhat = (Yhat,)
        with self.file_writer.as_default():
            for i,sl in zip(self.image_for_outputs,self.output_slices):
                # print('output', self.__wrapped_model.outputs[i], tuple(sl))
                output_shape = self.__wrapped_model.output_shape if self.n_outputs==1 else self.__wrapped_model.output_shape[i]
                n_channels_out = sep = output_shape[-1]
                if self.prob_out: # first half of output channels is mean, second half scale
                    n_channels_out % 2 == 0 or _raise(ValueError())
                    sep = sep // 2
                ...
                if self.prob_out:
                    ...
                    scale_name = self._name('net_output_scale', self.__wrapped_model.outputs[i], i, self.n_outputs)
                    output_name = self._name('net_output_mean',  self.__wrapped_model.outputs[i], i, self.n_outputs)
                    tf.summary.image(output_name, output_image, step=epoch)
                    tf.summary.image(scale_name, scale_image, step=epoch)
                else:
                    output_name = self._name('net_output', self.__wrapped_model.outputs[i], i, self.n_outputs)
                    tf.summary.image(output_name, output_image, step=epoch)
        ...
        if epoch == 0:
            with self.file_writer.as_default():
                for i,sl in zip(self.image_for_outputs,self.output_slices):
                    # print('target', self.__wrapped_model.outputs[i], tuple(sl))
                    target_name = self._name('net_target', self.__wrapped_model.outputs[i], i, self.n_outputs)
